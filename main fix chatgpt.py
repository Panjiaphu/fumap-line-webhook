...
# ============================================================
# Fumap LINE Webhook V4.2 RichMenu E/F + OpenAI Vision
...
# - Image message: tải ảnh từ LINE rồi gửi OpenAI Vision phân tích TradingView chart.
# ============================================================
...
def line_auth_headers() -> Dict[str, str]:
...
def call_openai_image_analysis(line_user_id: str, image_bytes: bytes, member: Optional[Dict[str, Any]]) -> str:
    ...
    prompt = (
        ...
        "1. 目前市場狀態：多頭 / 空頭 / 區間\n"
        "2. 重要支撐與壓力區\n"
        "3. 指標重點：RSI、EMA、成交量、結構、BOS/CHOCH 如可辨識\n"
        "4. 操作劇本：LONG / SHORT / WAIT 三種情境\n"
        "5. 風險控管：進場條件、失敗條件、停損觀念\n\n"
        "注意：不要保證獲利，不要建議重倉，不要說一定會漲或一定會跌。"
    )
    ...
    try:
        ...
        try:
            ...
            print(f"[openai_vision] responses fallback: {responses_error}")
            ...
            answer = resp2.choices[0].message.content or "AI 無回覆。"
...
def handle_admin_command(line_user_id: str, reply_token: str, text: str) -> bool:
    ...
    if cmd == "check":
        lines = [
            ...
            f"OPENAI_VISION_MODEL: {OPENAI_VISION_MODEL}",
        ]
    ...
    if cmd == "free" and len(parts) >= 2:
        ...
        try:
            profile = line_profile(uid)
            name = profile.get("displayName", "")
            upsert_member(uid, name, "FREE", 1, note=f"admin {line_user_id}")
...
@app.get("/")
def home():
    return jsonify({"ok": True, "service": "Fumap LINE Webhook V4.2 RichMenu E/F + Vision", "time": now_tw()})
